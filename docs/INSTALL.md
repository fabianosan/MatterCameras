...
| **Docker** + Compose v2 | **Required for both paths** — Option A builds locally (`bash scripts/setup.sh`), Option B runs pre-built images (`docker compose -f docker-compose.ghcr.yml`) |
