...
| **SmartThings + Matter bridge** | **~50** bridged devices with good stability | Reports of instability above this in large installs ([1Home](https://tr.docs.netlify.1home.io/docs/en/server/matter-bridge/apps/samsung-smartthings)) |
| **SmartThings legacy cameras (SmartCam LAN)** | **4** cameras | **Does not apply** to Matter 1.5 cameras |
...
2. **WebRTC / TURN** — live view depends on ICE + SmartThings hub TURN; snapshots do not use WebRTC.
3. **Hub** — retries `ProvideOffer` if media does not connect within ~30 s.
...
- Limit or warn when adding more than **4 cameras** in the Web UI.
- Use an **Aeotec / SmartThings standalone** hub with Matter 1.5 camera firmware (not a TV-embedded hub).
